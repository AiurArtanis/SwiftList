...
public class AnimeThemeProvider : IThemeProvider
{
    public string Name => "Anime Themes";

    public IEnumerable<ITheme> GetThemes()
    {
        var themes = new List<ITheme>();
        var assembly = typeof(AnimeThemeProvider).Assembly;
                ...
                foreach (System.Collections.DictionaryEntry entry in reader)
                {
                        ...
                        key.EndsWith(".baml", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                themes.Add(new AnimeTheme(id, isDark, packUri));
                            }
                        }
                        catch
                        {
...
public class AnimeTheme : ITheme
{
    ...
    public AnimeTheme(string id, bool isDark, string packUri)
    {
        Id = id;
        IsDark = isDark;
        _packUri = packUri;
