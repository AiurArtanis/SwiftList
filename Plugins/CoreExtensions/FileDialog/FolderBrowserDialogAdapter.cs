...
namespace SwiftList.Plugins.CoreExtensions.FileDialog
{
    public class FolderBrowserDialogAdapter : IFileDialogAdapter
    {
        ...
        public bool CanHandle(IntPtr hwnd, string className, string processName)
        {
            ...
            IntPtr treeView = FindTreeView(hwnd);
            ...
            int ctrlId = GetDlgCtrlID(treeView);
        ...
        public bool NavigateTo(IntPtr hwnd, string targetPath)
        {
            try
            {
                ...
                IntPtr hProcess = OpenProcess(PROCESS_VM_OPERATION | PROCESS_VM_WRITE, false, (int)pid);
                if (hProcess == IntPtr.Zero) return false;

                try
                {
                    byte[] pathBytes = Encoding.Unicode.GetBytes(targetPath + "\0");
                    uint size = (uint)pathBytes.Length;

                    IntPtr remoteMem = VirtualAllocEx(hProcess, IntPtr.Zero, size, MEM_COMMIT, PAGE_READWRITE);
        ...
        {
            IntPtr tree = FindWindowEx(parent, IntPtr.Zero, "SysTreeView32", null);
            if (tree != IntPtr.Zero) return tree;

            IntPtr child = FindWindowEx(parent, IntPtr.Zero, null, null);
            while (child != IntPtr.Zero)
            {
                IntPtr subTree = FindTreeView(child);
