...
public class FileSizeFilterProvider : ISidebarFilterProvider
{
    ...
    public IEnumerable<SidebarFilterGroup> GetFilterGroups()
    {
        ...
        group.Items.Add(new SidebarFilterItem
        {
            ...
            FilterPredicate = results => FilterBySizeAsync(results, size => size < 1 * 1024 * 1024)
        });
        ...
        group.Items.Add(new SidebarFilterItem
        {
            ...
            FilterPredicate = results => FilterBySizeAsync(results, size => size >= 1 * 1024 * 1024 && size <= 100 * 1024 * 1024)
        });
        ...
        group.Items.Add(new SidebarFilterItem
        {
            ...
            FilterPredicate = results => FilterBySizeAsync(results, size => size > 100 * 1024 * 1024)
        });

        return new[] { group };
    }

    private static async Task<IReadOnlyList<ISearchResult>> FilterBySizeAsync(IReadOnlyList<ISearchResult> results, Func<long, bool> matches)
    {
        var files = results.Where(r => !r.IsDir).ToList();
        var paths = files.Select(r => r.FullPath).Distinct().ToList();
        var metadata = await FileMetadataService.GetMetadataAsync(paths);
        return files.Where(r => metadata.TryGetValue(r.FullPath, out var m) && matches(m.Size)).ToList();
    }
}
