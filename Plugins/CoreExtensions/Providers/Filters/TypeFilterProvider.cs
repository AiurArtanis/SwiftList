...
namespace SwiftList.Plugins.CoreExtensions.Providers.Filters;
...
public class TypeFilterProvider : ISidebarFilterProvider
{
    ...
    public IEnumerable<SidebarFilterGroup> GetFilterGroups()
    {
        ...
        group.Items.Add(new SidebarFilterItem
        {
            ...
            FilterPredicate = results => Task.FromResult<IReadOnlyList<ISearchResult>>(results.Where(res => !res.IsApplication && res.IsDir).ToList())
        });
        ...
        group.Items.Add(new SidebarFilterItem
        {
            ...
            FilterPredicate = results => Task.FromResult<IReadOnlyList<ISearchResult>>(results.Where(res => !res.IsApplication && !res.IsDir && DocExts.Contains(Path.GetExtension(res.FullPath))).ToList())
        });
        ...
        group.Items.Add(new SidebarFilterItem
        {
            ...
            FilterPredicate = results => Task.FromResult<IReadOnlyList<ISearchResult>>(results.Where(res => !res.IsApplication && !res.IsDir && ImageExts.Contains(Path.GetExtension(res.FullPath))).ToList())
        });
        ...
        group.Items.Add(new SidebarFilterItem
        {
            ...
            FilterPredicate = results => Task.FromResult<IReadOnlyList<ISearchResult>>(results.Where(res => !res.IsApplication && !res.IsDir && VideoExts.Contains(Path.GetExtension(res.FullPath))).ToList())
        });
