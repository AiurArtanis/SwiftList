    ...
    public class ShellMenuSession : IDisposable
    {
        ...
        public static ShellMenuSession? Create(string path)
        {
            ...
            try
            {
                ...
                if (hr < 0)
                {
                    Logger.Log($"[ShellMenuSession] SHParseDisplayName failed for: {path} (HR: {hr})");
                    session.Dispose();
                    return null;
                }
                ...
                if (hr < 0)
                {
                    Logger.Log($"[ShellMenuSession] SHBindToParent failed (HR: {hr})");
                    session.Dispose();
                    return null;
                }
            ...
            {
                Logger.Log($"[ShellMenuSession] Create failed: {ex.Message}");
                session.Dispose();
                return null;
            }
        }
        ...
        public void InvokeCommand(uint commandId, IntPtr ownerHwnd)
        {
            ...
            {
                Logger.Log($"[ShellMenuSession] InvokeCommand failed (id={commandId}): {ex.Message}");
            }
        }
