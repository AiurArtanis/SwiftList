...
public class BrowserDataInstantProvider : IInstantResultProvider
{
    ...
    {
        var value = PluginSettingsService.GetSetting("SwiftList.Plugins.BrowserData", "TriggerKeyword", "bm");
        return string.IsNullOrWhiteSpace(value) ? "bm" : value;
    }

    // Requires an explicit "<keyword> <search term>" prefix (mirrors ProcessManager's "ps <term>")
    // rather than matching on every query -- a large cache of bookmarks/history would otherwise compete
    // for attention against every unrelated search, and scanning it is real per-keystroke cost (see
    // CollectMatches) that shouldn't be paid unless the user actually asked for a browser-data search.
