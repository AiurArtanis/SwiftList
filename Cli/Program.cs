using SwiftList.Cli.Search;
using SwiftList.Cli.Ui;
...
// deliberately out of scope. Requires the real SwiftList App to be running.
//
...
//  - args[0], if present, becomes the initial query (`slf report` opens already filtered to "report").
//  - Otherwise, if stdin is redirected (`echo report | slf`), its first non-empty line becomes the
...
// -- where this exe already sits alongside SwiftList.App.exe -- straight to the machine's PATH), not this
...
{
    Console.Error.WriteLine($"[error] Could not reach the SwiftList App's search pipe within {AppSearchPipeClient.ConnectTimeoutMs}ms ({ex.GetType().Name}: {ex.Message}) -- is the SwiftList App running?");
    return;
}

var terminal = SwiftList.Cli.Terminal.Terminal.TryOpen();
if (terminal == null)
...
while (true)
{
    ...
    // Left/Right/Home/End move the text cursor within the query -- matches the split the real SwiftList
