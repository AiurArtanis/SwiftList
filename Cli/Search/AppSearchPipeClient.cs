...
public static class AppSearchPipeClient
{
    ...
    public static string PipeNameFor(string userName) => $"SwiftList_App_Search_Pipe_{userName}";

    // Verified reachable BEFORE the interactive UI ever opens a console handle: a script or pipeline that
    // invokes `slf` non-interactively (the whole point of the ffmpeg/`for /f` usage this was built for)
    ...
    public static async Task ProbeAsync(string pipeName)
    {
        ...
        // of Administrators -- so `slf` run from an admin terminal was throwing UnauthorizedAccessException
    ...
    // SwiftListPipe) -- avoids interleaving a newer query's request bytes with an older query's still-
    ...
    public static async Task<List<(SearchResult Result, int[] Highlights)>> SearchAsync(string pipeName, string query, Action<List<(SearchResult, int[])>> onProgress, CancellationToken token)
    {
        ...
        // `slf` fail its own redundant client-side owner check against a non-elevated App).
