...
namespace SwiftList.Cli.Terminal;
...
// path(s) printed on Enter -- so `slf | xargs code` or `$path = & slf.exe` works the same way piping
...
//
// Input: stdin can likewise be redirected to supply the CLI's initial query (`echo report | slf`), which
