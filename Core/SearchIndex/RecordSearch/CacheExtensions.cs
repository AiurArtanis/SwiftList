...
namespace SwiftList.Core.SearchIndex.RecordSearch
{
    internal static class CacheExtensions
    {
            ...
            ulong? directoryRootId)
        {
            ...
            CandidateCacheEntry? previous = searcher.CacheManager.GetCandidateCache(index, cacheTerm);
            if (previous == null ||
            ...
            foreach (int i in previous.Candidates)
            {
            ...
            int[] candidates)
        {
            ...
            int chunkCount = (candidates.Length + Helpers.NameSearchChunkSize - 1) / Helpers.NameSearchChunkSize;
            ...
            Parallel.For(
                ...
                (chunk, _, worker) =>
                {
                    ...
                    int end = Math.Min(start + Helpers.NameSearchChunkSize, candidates.Length);
        ...
        public static bool TryGetRankCache(this Searcher searcher, RuntimeIndex index, string cacheTerm, int limit, out FzfRank[] ranks)
        {
            return searcher.CacheManager.TryGetRankCache(index, cacheTerm, limit, out ranks);
        }

        public static void StoreRankCache(this Searcher searcher, RuntimeIndex index, string cacheTerm, List<FzfRank> ranks, int limit)
        {
            searcher.CacheManager.StoreRankCache(index, cacheTerm, ranks, limit);
        }
    }
}
