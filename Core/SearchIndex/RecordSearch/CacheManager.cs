...
internal sealed class CandidateCacheStore
{
    ...
    public CandidateCacheStore(int count) => Count = count;
    ...
    public void Store(CandidateCacheEntry entry)
    {
        ...
        while (_entries.Count > MaxCandidateCacheTermsPerSource && _order.TryDequeue(out var oldest))
...
internal sealed class CacheManager
{
    ...
    public bool TryGetRankCache(RuntimeIndex index, string cacheTerm, int limit, out FzfRank[] ranks) => _rankCache.TryGet(index.SourceKey, index.Count, cacheTerm, limit, out ranks);

    public void StoreRankCache(RuntimeIndex index, string cacheTerm, List<FzfRank> ranks, int limit) => _rankCache.Store(index.SourceKey, index.Count, cacheTerm, limit, ranks.ToArray());
