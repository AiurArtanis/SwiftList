...
namespace SwiftList.Core.SearchIndex.RecordSearch
{
    internal static class Helpers
    {
        ...
        {
            if (cache.TryGetValue(entryIndex, out bool cached))
                return cached;

            bool result = index.IsUnderDirectoryIndex(entryIndex, directoryRootIndex);
            cache[entryIndex] = result;
            return result;
        }

        public static FzfPatternResult ToPatternResult(FzfMatchResult match)
        {
            return new FzfPatternResult(match.Score, match.Start, match.End, match.End, match.IsMatch);
        }
        ...
        {
            int entryIndex = rank.EntryIndex;
        ...
        public static ulong? TryGetDirectoryRootId(this RuntimeIndex index, string? directoryFilterLower)
        {
            ...
            string sourceRootLower = index.SourceKey.ToLowerInvariant() + @":\";
        ...
        public static void RequestBackgroundCompaction()
        {
            ...
            long last = Interlocked.Read(ref _lastBackgroundCompactionTicks);
