...
internal static class FzfFuzzyMatcher
{
    public static FzfMatchResult FuzzyMatchV2(string text, string pattern, bool caseSensitive, FzfScoringScheme scheme, FzfSlab? slab = null)
    {
        ...
        var previousClass = FzfAlgorithm.InitialClass(scheme);
        for (var offset = 0; offset < scopedLength; offset++)
        {
            var raw = text[minIdx + offset];
            var currentClass = FzfAlgorithm.GetClass(raw);
            var normalized = FzfAlgorithm.NormalizeChar(raw, caseSensitive);
