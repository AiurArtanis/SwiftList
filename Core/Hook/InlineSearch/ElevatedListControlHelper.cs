...
internal static class ElevatedListControlHelper
{
    public static List<string> GetListItems(IntPtr hwnd)
    {
        ...
        {
            var count = (int)ListSearchNativeMethods.SendMessage(hwnd, ListSearchNativeMethods.LB_GETCOUNT, IntPtr.Zero, IntPtr.Zero);
            if (count <= 0) return result;

            for (var i = 0; i < count; i++)
            {
                var textLen = (int)ListSearchNativeMethods.SendMessage(hwnd, ListSearchNativeMethods.LB_GETTEXTLEN, (IntPtr)i, IntPtr.Zero);
                if (textLen > 0)
                {
        ...
        {
            var count = (int)ListSearchNativeMethods.SendMessage(hwnd, ListSearchNativeMethods.LVM_GETITEMCOUNT, IntPtr.Zero, IntPtr.Zero);
            if (count <= 0) return result;

            // Query multi-column headers
            var colCount = 1;
            var hwndHeader = ListSearchNativeMethods.SendMessage(hwnd, 0x1000 + 31, IntPtr.Zero, IntPtr.Zero); // LVM_GETHEADER
            if (hwndHeader != IntPtr.Zero)
            {
                var headerItems = (int)ListSearchNativeMethods.SendMessage(hwndHeader, 0x1200 + 0, IntPtr.Zero, IntPtr.Zero); // HDM_GETITEMCOUNT
                if (headerItems > 1)
                {
            ...
            ListSearchNativeMethods.GetWindowThreadProcessId(hwnd, out var pid);
            var hProcess = ListSearchNativeMethods.OpenProcess(
            ...
            var lvItemSize = (uint)Marshal.SizeOf<ListSearchNativeMethods.LVITEM>();
            uint bufferSize = 512;
            var remoteLvItemPtr = ListSearchNativeMethods.VirtualAllocEx(hProcess, IntPtr.Zero, lvItemSize, ListSearchNativeMethods.MEM_COMMIT, ListSearchNativeMethods.PAGE_READWRITE);
            var remoteBufferPtr = ListSearchNativeMethods.VirtualAllocEx(hProcess, IntPtr.Zero, bufferSize, ListSearchNativeMethods.MEM_COMMIT, ListSearchNativeMethods.PAGE_READWRITE);
            ...
            if (remoteLvItemPtr != IntPtr.Zero && remoteBufferPtr != IntPtr.Zero)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    for (var col = 0; col < colCount; col++)
                    {
                        ...
                        var text = Encoding.Unicode.GetString(localBuffer);
    ...
    public static void SelectItem(IntPtr hwnd, string className, int index, bool clearOthers, bool selectState)
    {
        ...
        var style = ListSearchNativeMethods.GetWindowLong(hwnd, ListSearchNativeMethods.GWL_STYLE);
        var isMulti = false;
        if (className.Contains("ListBox", StringComparison.OrdinalIgnoreCase))
        {
        ...
        {
            ListSearchNativeMethods.GetWindowThreadProcessId(hwnd, out var pid);
            var hProcess = ListSearchNativeMethods.OpenProcess(ListSearchNativeMethods.PROCESS_VM_OPERATION | ListSearchNativeMethods.PROCESS_VM_WRITE, false, pid);
    ...
    public static List<int> GetSelectedIndices(IntPtr hwnd, string className)
    {
        ...
        {
            var style = ListSearchNativeMethods.GetWindowLong(hwnd, ListSearchNativeMethods.GWL_STYLE);
            var isMulti = (style & (int)ListSearchNativeMethods.LBS_MULTIPLESEL) != 0 ||
                           (style & (int)ListSearchNativeMethods.LBS_EXTENDEDSEL) != 0;
            if (isMulti)
            {
                var selCount = (int)ListSearchNativeMethods.SendMessage(hwnd, LB_GETSELCOUNT, IntPtr.Zero, IntPtr.Zero);
                if (selCount > 0)
                {
            ...
            {
                var curSel = (int)ListSearchNativeMethods.SendMessage(hwnd, ListSearchNativeMethods.LB_GETCURSEL, IntPtr.Zero, IntPtr.Zero);
                if (curSel >= 0)
                {
                    selected.Add(curSel);
                }
    ...
    {
        var parent = ListSearchNativeMethods.GetParent(hwnd);
        if (parent == IntPtr.Zero) return;

        var ctrlId = ListSearchNativeMethods.GetWindowLong(hwnd, ListSearchNativeMethods.GWL_ID);
        var wParam = (IntPtr)(((int)notificationCode << 16) | (ctrlId & 0xFFFF));
