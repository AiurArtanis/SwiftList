...
namespace SwiftList.Core.Hook
{
    public sealed class HookIpcClient : IDisposable
    {
        ...
        private async Task RunLoop(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        IpcMessage msg = await PipeRequestBinarySerializer.ReadMessageAsync(eventPipe, token).ConfigureAwait(false);
                        DispatchEvent(msg);
                    }
                }
        ...
        {
            return HookProcessLauncher.Launch(_serviceExePath, _autoElevate);
        }
