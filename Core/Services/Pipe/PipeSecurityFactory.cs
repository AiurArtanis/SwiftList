...
{
    // SwiftListPipe's own pipe (UsnServicePipeServer): deliberately broad. That pipe is hosted by the
    ...
    // (e.g. `slf` run from an admin terminal) trips the CLIENT-side CurrentUserOnly check against a
