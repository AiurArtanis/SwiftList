...
// SwiftList.App.exe, then asks HookProcessBroker to spawn the hook into that caller's own session --
...
internal static class HookLaunchRequestHandler
{
    public static PipeResponse Handle(NamedPipeServerStream pipe, bool requestElevation)
    {
        try
        {
            ...
            {
                Logger.Log($"[UsnService] Rejected LaunchHook: PID {callerPid} is not this install's SwiftList.App.exe.", LogLevel.Warn);
    ...
    private static bool IsGenuineAppProcess(int pid)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(callerPath) ||
                !Path.GetFileName(callerPath).Equals("SwiftList.App.exe", StringComparison.OrdinalIgnoreCase))
                return false;
