...
namespace SwiftList.Core.Indexer.NetworkDrive
{
    internal sealed class TreeBuilder
    {
        ...
        {
            NetworkIgnoreRuleSet ignoreRules = _filter.LoadIgnoreRules(current.Path, current.LogicalPath, current.IgnoreRules);
            ...
            foreach (string child in children)
            {
                ...
                var createResult = TryCreateRecord(child, current.LogicalPath, current.LocalId, out var record, out bool isDirectory, out string logicalFullPath);
                ...
                int indexedItems = Interlocked.Increment(ref _indexedItems);
        ...
        private WalkRecordResult TryCreateRecord(string child, string logicalParentPath, UInt128 parentId, out NetworkWalkRecord record, out bool isDirectory, out string fullPath)
        {
            ...
            string name = Path.GetFileName(child.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
            if (string.IsNullOrEmpty(name))
                return WalkRecordResult.InvalidName;

            string logicalPath = Path.Combine(logicalParentPath, name);
            fullPath = PathHelpers.NormalizePath(logicalPath, isDirectory);
            ulong id = PathHelpers.HashPath64(fullPath);
        ...
        private void MaybeCheckpoint(int indexedItems)
        {
            ...
            int count = Interlocked.Increment(ref _countSinceCheckpoint);
            bool countDue = count >= CheckpointBatchSize;
            bool timeDue = new TimeSpan(nowTicks - Interlocked.Read(ref _lastCheckpointTicks)) >= CheckpointInterval;
            if (!countDue && !timeDue)
                return;
        ...
        private NetworkDriveWalkStats CurrentStats()
        {
            return new NetworkDriveWalkStats(
                Volatile.Read(ref _skippedItems),
                Volatile.Read(ref _errors),
