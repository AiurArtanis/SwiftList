        ...
        while (!_token.IsCancellationRequested)
        {
            try
            {
                ...
                if (!success)
                {
                    ...
                    Logger.Log($"[Monitor] FSCTL_READ_USN_JOURNAL error on drive {_drive}: {err}", LogLevel.Error);
                    // Don't immediately break; wait and retry a few times
                    await Task.Delay(2000, _token);
                    consecutiveEmptyReads++;
                    if (consecutiveEmptyReads > 10)
                    {
                        Logger.Log($"[Monitor] Too many consecutive errors on drive {_drive}. Stopping monitor.", LogLevel.Error);
                        break;
                    }
                    continue;
                }
