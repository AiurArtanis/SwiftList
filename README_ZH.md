...
* **低功耗实时后台监控**：独立 Windows 后台服务 `SwiftListService` 负责监听 USN 变化，通过全新的**零分配二进制 IPC 通道**进行流式传输，增删改移实时在后台零感完成毫秒级同步，内存占用极低。
...
### 4. 📂 级联文件夹浏览器与空白处快捷导航 (Folder Cascader)
* **空白处双击/中键激活**：当在 Windows 资源管理器空白区域或桌面双击鼠标左键或按下鼠标中键时，会智能弹出极速级联文件夹菜单。
* **无限级目录深度下探**：支持无延迟地在级联菜单中无限级向下浏览子目录。配合 Win32 Shell API，可完整列出系统虚拟路径（如“快速访问”、“此电脑”、“回收站”等）。
* **收藏夹与最近历史**：插件深度集成了收藏夹和最近访问历史记录，用户可在设置面板中自由配置需要快捷跳转的路径，实现一键穿梭直达。

### 5. 🖥️ 三合一多维度交互视图
* **快速搜索窗口 (QuickSearchWindow)**：经典轻量的主流启动器面板。支持快捷呼出（**双击 Ctrl 键**），支持快捷键徽标（如 `Ctrl+1` 至 `Ctrl+9`）极速盲操选中。
* **内联上下文挂载窗口 (InlineSearchWindow)**：极为创新的“贴靠挂载”模式。可智能检测并直接贴合挂载在 Windows 资源管理器窗口（Explorer）、各类系统文件选择/保存对话框或第三方通用列表控件的上方，方便快速跳转、多屏协同与目录穿梭。
  * **原生资源管理器与 Directory Opus 支持**：深度优化键盘钩子，完美适配现代 Windows Explorer (`DirectUIHWND` / `SHELLDLL_DefView`) 及 **Directory Opus** 专业双栏资源管理器焦点。支持 Win11 动态二级子菜单加载。
* **控制面板与系统设置 (SettingsWindow)**：提供全面的可视化配置中心。支持插件配置的层级分组折叠展示，并提供包含排除规则、网盘挂载、**历史记录编辑器（查找、过滤、按需清理最近搜过与打开过的内容）**以及后台服务实时监控看板。
...
* **智能溢出展示与横向滚动**：针对超出容器宽度的多列文本，主窗口支持通过 `Shift + 滚轮` 触发横向平移；内联挂载窗口（悬浮窗）则会自动启用平滑的跑马灯滚动效果（Marquee）并隐藏滚动条，在选中或悬停时平顺循环展示完整内容，兼顾实用与极致整洁的 UI 空间。

### 7. 👁️ 模块化文件极速预览 (File QuickLook)
...
* **`ISearchResultAction`（动作菜单插件）**：定义文件搜索项的二级右键或右方向键操作行为。如：“打开所在文件夹”、“复制文件路径”。同时内置了专属于内联挂载窗口的命令。
...
* **二级菜单 (Actions Menu)**：
  * 选中某条搜索结果后，按下 `→` 方向键或点击右键，即可拉出针对该文件的动作菜单。
...
* **Tab 键穿梭**：
  ...
  * 选中您想去的文件夹，按下回车键，资源管理器将**瞬间平滑切换**到目标路径，彻底免去手动双击逐层寻找的烦恼。
* **保存对话框一键导航**：
