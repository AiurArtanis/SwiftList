...
public sealed class ExistenceCheckActionsTests
{
    ...
    {
        public string Path { get; } = System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"swiftlist-tests-{Guid.NewGuid():N}.txt");
    ...
    public void CopyFileAction_NonExistentFile_CanExecuteFalse() =>
        Assert.IsFalse(new CopyFileAction().CanExecute(new ISearchResult[] { new FakeResult { FullPath = @"Z:\definitely-not-real-swiftlist-file.txt" } }));
    ...
    public void OpenResultAsAdminAction_Directory_CanExecuteFalse()
    {
        ...
        var dir = Directory.CreateTempSubdirectory("swiftlist-tests-");
        try
        {
