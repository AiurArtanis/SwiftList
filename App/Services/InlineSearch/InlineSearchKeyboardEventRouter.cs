...
namespace SwiftList.App.Services
{
    ...
    internal sealed class InlineSearchKeyboardEventRouter
    {
        ...
        public void Wire()
        {
            _keyboardHook.OnCharacterTyped += ch =>
            {
            ...
            _keyboardHook.OnBackspacePressed += () =>
            {
                Application.Current.Dispatcher.BeginInvoke(new Action(() =>
                {
                    var window = _getWindow();
            ...
            _keyboardHook.OnUpPressed += () =>
            {
                Application.Current.Dispatcher.BeginInvoke(new Action(() =>
                {
                    ...
                    {
                        int prev = window.LstResults.SelectedIndex - 1;
                        if (prev >= 0)
            ...
            _keyboardHook.OnDownPressed += () =>
            {
                Application.Current.Dispatcher.BeginInvoke(new Action(() =>
                {
                    ...
                    if (window.LstResults.Items.Count > 0)
                    {
                        int next = window.LstResults.SelectedIndex + 1;
            ...
            _keyboardHook.OnCtrlNumberPressed += num =>
            {
                Application.Current.Dispatcher.BeginInvoke(new Action(() =>
                {
                    _getWindow()?.LaunchByShortcutIndex(num);
                }));
            };
        }
    }
