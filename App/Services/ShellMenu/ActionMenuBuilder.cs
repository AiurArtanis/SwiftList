            ...
            Dictionary<IntPtr, IDynamicActionProvider> subMenuToProviderMap)
        {
            ...
            if (hMenu == IntPtr.Zero)
            {
                ...
                foreach (var registration in PluginManager.Instance.Actions)
                {
                    ...
                    {
                        string group = string.IsNullOrWhiteSpace(action.GroupName) ? TranslationManager.Instance["Action_BuiltinGroup"] : action.GroupName;
                ...
                foreach (var provider in PluginManager.Instance.DynamicProviders)
                {
                    ...
                    if (provider.CanProvide(activeResult))
                    {
                        ...
                        {
                            string group = string.IsNullOrWhiteSpace(provider.GroupName) ? TranslationManager.Instance["Action_BuiltinGroup"] : provider.GroupName;
                            uiItems.Add(new ActionMenuItem
                            {
                                IsSectionHeader = true,
                                SectionTitle = group
