...
namespace SwiftList.App.Converters
{
    ...
    public static class TextHighlighter
    {
        ...
        private static void OnTextChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            string fullText = GetText(textBlock);
            ...
            string normalizedHighlight = NormalizePathSeparators(highlight.Trim()).ToLowerInvariant();

            if (ContainsPathSeparator(normalizedHighlight))
            {
                if (TryNormalizeDrivePath(normalizedHighlight, out _, out string normalizedDrivePath))
                {
            ...
            string fullTextLower = fullText.ToLowerInvariant();
            ...
            foreach (var term in terms)
            {
                ...
                {
                    int foundIdx = fullTextLower.IndexOf(termLower, startIdx, StringComparison.Ordinal);
                    if (foundIdx < 0) break;

                    for (int i = foundIdx; i < foundIdx + termLower.Length && i < highlights.Length; i++)
                        highlights[i] = true;
            ...
            {
                bool isHighlighted = highlights[pos];
                int end = pos;
                while (end < fullText.Length && highlights[end] == isHighlighted)
                    end++;

                string segment = fullText.Substring(pos, end - pos);
