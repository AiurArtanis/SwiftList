...
A search box sits in the Settings window's title bar. Type to get a live-filtered popup of matching
settings across every section — including the per-plugin entries under Plugins, Hotkeys' Plugin
Actions tab, and Startup Panel's Plugin Tabs sub-tab. Each result shows a breadcrumb (e.g. "Index >
Network Drives") so same-named settings under different tabs stay distinguishable. Selecting a
result (click, or Up/Down to highlight and Enter) switches to the right section and tab, scrolls the
exact control into view, and briefly flashes a highlight border around it.

The Settings window has nine sections in its left sidebar:

| Section | Covers |
|---|---|
| [Service Status](./service-status) | Background service install/start/stop, and the App/Hook/Service log viewer. |
